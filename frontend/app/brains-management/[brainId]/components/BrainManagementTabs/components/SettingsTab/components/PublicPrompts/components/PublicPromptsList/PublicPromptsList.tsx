  ...
  return (
    <div ref={selectRef} className="relative min-w-[200px] inline-block">
      <button
        ...
        type="button"
        className="px-4 py-2 w-full text-gray-700 bg-secondary border rounded-md focus:outline-none focus:border-blue-500"
      >
        {selectedOption ? selectedOption.title : t("selectQuivrPersonalityBtn", { ns: "config" })}
      </button>
      {isOpen && (
        <div className="absolute top-10 w-full bg-secondary border rounded-md shadow-lg z-10">
          {options.map((option) => (
            <div
              key={option.id}
