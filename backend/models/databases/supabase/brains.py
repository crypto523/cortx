...
class Brain(Repository):
    ...
    def create_brain(self, brain: CreateBrainProperties): # type: ignore
        response = (self.db.table("brains").insert(brain.dict())).execute()
        return BrainEntity(**response.data[0])

    def get_user_brains(self, user_id) -> list[MinimalBrainEntity]: # type: ignore
        response = (
    ...
    def get_brain_for_user(self, user_id, brain_id) -> MinimalBrainEntity | None: # type: ignore
        response = (
            self.db.from_("brains_users")
    ...
    def delete_brain_user_by_id( # type: ignore
        self,
        user_id: UUID,
        brain_id: UUID,
    ):
    ...
    def get_brain_vector_ids(self, brain_id): # type: ignore
        """
    ...
    def delete_file_from_brain(self, brain_id, file_name: str): # type: ignore
    ...
    def get_default_user_brain_id(self, user_id: UUID) -> UUID | None: # type: ignore
        response = (
            (
                self.db.from_("brains_users")
                .select("brain_id")
    ...
    def get_brain_by_id(self, brain_id: UUID) -> BrainEntity | None: # type: ignore
        response = (
            self.db.from_("brains")
            .select("id:brain_id, name, *")
            .filter("brain_id", "eq", brain_id)
